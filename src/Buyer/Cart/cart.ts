...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (document.getElementsByClassName('navbar-brand')[0] as HTMLAnchorElement).href = "../allProduct/allProduct.html"
  ...
  document.getElementsByClassName('clear-cart-button')[0].addEventListener('click', async ()=>{
    await clearCart();
    updateBadgeCount();
    displayCartItems();
...
async function displayCartItems() {
  ...
  try {
    ...
    if (data[0].cart.length === 0) {
      ...
      (document.getElementsByClassName('clear-cart-button')[0] as HTMLButtonElement).disabled = true;
      (
    ...
    document.getElementsByClassName('total-price')[0].textContent = `${priceSum}`;
